...
def positive_common_ansers_per_group(answers_list: list) -> list:
    ...
    """
    return [
        len(functools.reduce(lambda A, B: A & B, answer)) for answer in answers_list
    ]
...
def main():
    ...
    print(
        f"Amount of total positive answers per group, sumemd all up: {sum(positive_answers)}"
    )

    parsed_data_v2 = parse_answers_v2(raw_data)
    unique_answers = positive_common_ansers_per_group(parsed_data_v2)
    print(
        f"Amount of total common positive answers per grop summd: {sum(unique_answers)}"
    )
