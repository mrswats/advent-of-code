...
def check_password_v2(string: str) -> bool:
    if (parsed := parse_regex) :
        ...
        return operator.xor(passwd[int(lower) - 1] == token, passwd[int(higher) - 1] == token)

    return False


def main():
    print("Correct passwords (v1): ", [check_password_v1(string) for string in read_input()].count(True))
    print("Correct passwords (v2): ", [check_password_v2(string) for string in read_input()].count(True))
